...
from estilos_ui import obtener_estilos_completos
...
class AsistenteVirtualModernUI(QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            # Limpiar configuración de Notion al salir
            self.limpiar_configuracion_notion()
    ...
    def limpiar_configuracion_notion(self):
        """Limpiar la configuración de Notion al cerrar la aplicación"""
        try:
            from notion_integration import NotionConfigManager
            
            config_manager = NotionConfigManager()
            
            # Solo limpiar si está configurado
            if config_manager.esta_configurado():
                if config_manager.limpiar_configuracion():
                    print("🔒 Configuración de Notion limpiada por seguridad al cerrar la aplicación")
                else:
                    print("⚠️ No se pudo limpiar completamente la configuración de Notion")
                    
        except Exception as e:
            print(f"❌ Error al limpiar configuración de Notion: {e}")
    
    def apply_modern_styles(self):
        """Aplicar estilos modernos CSS desde archivo centralizado"""
        self.setStyleSheet(obtener_estilos_completos())
